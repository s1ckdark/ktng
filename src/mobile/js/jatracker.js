...
if (typeof jatracker === 'function') {
  window.addEventListener('load', function () {
    ...
    $('#nav .nav-menu').on('click', function () {
      jatracker('event', 'clickEvent', '링크', '클릭', '네비게이션 열기');
    });
    ...
    $('.tab-color-cosmetics .tab-nav a').on('click', function () {
      jatracker('event', 'clickEvent', '링크', '클릭', '색조 화장품 고르기-' + ($(this).data('name') ? $(this).data('name') : $(this).text()));
    });
    // 후각 - TPO 향수 사용법 탭
    $('.tab-perfume-tpo .tab-nav a').on('click', function () {
      jatracker('event', 'clickEvent', '링크', '클릭', 'TPO 향수 사용법-' + ($(this).data('name') ? $(this).data('name') : $(this).text()));
    });
    // 미각 - 계절별 추천차 탭
    $('.tab-tea .tab-nav a').on('click', function () {
      jatracker('event', 'clickEvent', '링크', '클릭', '계절별 추천차-' + ($(this).data('name') ? $(this).data('name') : $(this).text()));
    });
    // 촉각 - 보습케어 탭
    $('.layering-tab .tab-nav a').on('click', function () {
      jatracker('event', 'clickEvent', '링크', '클릭', '보습케어-' + ($(this).data('name') ? $(this).data('name') : $(this).text()));
    });
    // 촉각 - 피로풀기 탭
    $('.massage-tab .tab-nav a').on('click', function () {
      jatracker('event', 'clickEvent', '링크', '클릭', '피로풀기-' + ($(this).data('name') ? $(this).data('name') : $(this).text()));
    });
    // 청각 - 전화예절 탭
    $('.phone-tab .tab-nav a').on('click', function () {
      jatracker('event', 'clickEvent', '링크', '클릭', '전화예절-' + ($(this).data('name') ? $(this).data('name') : $(this).text()));
    });
    ...
    // 헤더 공유
    $('#snsMore .sns-list a').on('click', function(){
      var $this = $(this);
